function [wbest,hbest,normbest] = nnmf_mww(a,k,varargin)
...
narginchk(2,Inf);%检查参数数量是否在指定范围内
[n,m] = size(a);
...
[alg,w0,h0,tries,options] = ...
        internal.stats.parseArgs(pnames,dflts,varargin{:});%解析用户输入的命令行参数
    
% Check optional arguments
alg = internal.stats.getParamVal(alg,{'mult' 'als'},'ALGORITHM');%解析获取alg结构体mult als的信息
...
defaultopt = statset('nnmf');%设置统计分析工具箱
tolx = statget(options,'TolX',defaultopt,'fast');%从指定的选项结构体中获取参数值
...
%useParallel 是否使用并行计算，RNGscheme 随机数生成方案，poolsz 并行池的大小。options 输入的选项，true 解析并行计算选项。
...
if usePool
    ...
    pctRunOnAll internal.stats.parallel.muteParallelStore('rankDeficientMatrix', warning('off','MATLAB:rankDeficientMatrix') );
else
    % On client
