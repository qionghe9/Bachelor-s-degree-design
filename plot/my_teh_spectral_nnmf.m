function [ res ] = my_teh_spectral_nnmf( S )
    ...
    if isfield(S,'nnmf_psd_specs') && isfield(S,'nnmf_psd_maps')
        ...
        psdtmp=[];
        for kk=1:NK
            psdtmp=cat(2,psdtmp, squeeze(mean(abs(auto_spectra_comps(:,kk,:,:)),1))); %90x816(12*68)     
        end
    
        if ~isfield(S,'fixed_psd_specs')
            [a b]=nnmf_mww(psdtmp,maxP,'replicates',500,'algorithm','als');%90 4 4 816
        else
        ...
        for pp=1:maxP
            ind=1;
            for kk=1:NK
                maps(kk,pp,:)=b(pp,ind:ind+num_nodes-1); %12 4 8
                ind=ind+num_nodes;
            end
            specs(pp,:)=a(:,pp)';%4 90
        end
        ...
        [x,ind]=max(specs,[],2); %每行最大值的列数
        [x,neworder_auto]=sort(ind);
    
        res.nnmf_psd_specs=specs(neworder_auto,:);%低频到高频
    ...
    for kk=1:NK
        ...
        cohabs=squeeze(mean(abs(coh_comps(:,kk,:,:,:)),1)); % 90 68 68
        clear psd2;
        for pp=1:size(cohabs,1)
            psdf=triu(squeeze(cohabs(pp,:,:)),1);
            [inds]=(psdf~=0);
            psd2(pp,:)=psdf(inds); %90 2278
        end
        num_per_kk=size(psd2,2);
        
        psdtmp=cat(2,psdtmp, psd2);%90x27336(12*2278)
    ...
    % save('/GPFS/liuyunzhe_lab_permanent/heqiong/meg-data/SPM_ROI_DATA/test/adults/store/res.mat','res','-v7.3');
