...
if exist('embed') && embed.do,    
    ...
    if isfield(embed,'centre_freq')
        ...
        num_embeddings=round(span/embed.tres);  %窗口个数
    elseif isfield(embed,'num_embeddings')
        num_embeddings=embed.num_embeddings; 
    end

    lags=round(linspace(-num_embeddings/2,num_embeddings/2,num_embeddings)); %窗口采样点的位置差
