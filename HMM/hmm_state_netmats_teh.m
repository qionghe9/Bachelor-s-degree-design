function [ state_netmats ] = hmm_state_netmats_teh( hmm, S )
...
for subnum = 1:nsubs
    ...
    if ~S.global_only && hmm.K>1
        ...
        hmm_sub = hmm; 
        if subnum==length(D_fnames)
            hmm_sub.statepath = hmm.statepath(hmm.subj_inds(1:size(hmm.statepath,1))==subnum); 
            hmm_sub.gamma = hmm.gamma(hmm.subj_inds(1:size(hmm.statepath,1))==subnum,:);
        else
